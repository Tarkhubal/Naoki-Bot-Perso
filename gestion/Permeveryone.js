...
module.exports = {
    ...
    usage: 'peveryone',
    category: "owner",
    description: `Permet de désactiver toutes les permissions everyone du serveur.`,
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(pgp.fetch(`permgp_${message.guild.id}`)) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            const embed = new Discord.MessageEmbed()
                .setColor(color)
                .setTitle(`${message.author.tag} a désactivé toutes les __permissions everyone__ du serveur`)
